...
if [ "$outputtype" == "Aonly" ]; then
    if [[ ! $(ls "$systemdir/system/etc/init/" | grep *environ*) ]]; then
        ...
        echo "# AUTOGENERATED FILE BY KELEXINE GSI TOOLS" > "$systemdir/system/etc/init/init.treble-environ.rc"
...
date=`date +%Y%m%d`
outputname="$romtypename-$outputtype-$sourcever-$date-kelexine"
...
sed -i "s/$bdisplay/$displayid2=Built\.with\.kelexine\.Tools/" $systemdir/system/build.prop

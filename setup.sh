...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    if [[ "$distro" == "arch" || "$id_like" == "arch" ]]; then
       ...
       sudo apt install unace unrar zip unzip p7zip-full p7zip-rar sharutils rar uudeview mpack arj cabextract file-roller device-tree-compiler liblzma-dev python3 brotli liblz4-tool gawk aria2
    fi
